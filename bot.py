...
def get_users():
    users = {}
    for doc in db.collection("bot_users").stream():
        users[doc.id] = doc.to_dict()
    return users

def save_user(uid, data):
    db.collection("bot_users").document(uid).set(data)
...
def remove_track(title):
    for doc in db.collection("bot_tracks").where("title","==",title).stream():
...
def save_ratings(data):
    db.collection("bot_ratings").document("data").set(data)
...
async def start(update: Update, context):
    ...
    u = users[uid]
    if u.get("banned"):
        await update.message.reply_text("🚫 Забанены."); return
...
async def button(update: Update, context):
    ...
    if q.data == "open":
        ...
        if idx < len(u["collection"]) and not u["collection"][idx].get("rated"):
            ...
            rats = ratings.get(t, [])
            rats.append(r); ratings[t] = rats; save_ratings(ratings)
...
async def handle_text(update: Update, context):
    ...
    if update.effective_user.id == ADMIN_ID:
        ...
        if "pending_file_id" in context.user_data:
            ...
            fid = context.user_data["pending_file_id"]
            kb = [[InlineKeyboardButton(r.capitalize(),callback_data=f"addrar_{r}")] for r in ["common","rare","epic","legendary"]]
            context.user_data["new_title"] = title.strip()
            context.user_data["new_artist"] = artist.strip()
        ...
        if text.startswith("/ban "):
            ...
            for i,u in users.items():
                if u.get("username","")==t or u.get("nick","")==t:
                    u["banned"]=True; save_user(i,u); await update.message.reply_text(f"🚫 {t}"); return
            await update.message.reply_text("Не найден."); return
        if text.startswith("/unban "):
            t = text[7:].replace("@","")
            for i,u in users.items():
                if u.get("username","")==t or u.get("nick","")==t:
                    u["banned"]=False; save_user(i,u); await update.message.reply_text(f"✅ {t}"); return
            await update.message.reply_text("Не найден."); return
